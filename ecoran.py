...
class PowerManager(xAppBase):
        ...
        for node_id_str in nodes: # node_id_str is the GNB_ID from config
            ...
            try:
                ...
                self.e2sm_kpm.subscribe_report_service_style_1(
                    ...
                    report_interval_ms=kpm_report_p,
                    meas_names=metrics,
                    granularity_period_ms=kpm_gran_p,
                    callback=self._kpm_indication_callback # Pass the method directly
                )
    ...
    def run_power_management_xapp(self):
        ...
        try:
            ...
            # --- Main Control Loop ---
            while not self.is_terminated(): # Check termination flag from xAppBase
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Setup signal handling from xAppBase (if available) or a simple one
        def graceful_shutdown(signum, frame):
            print(f"Signal {signum} received, initiating shutdown...")
            if manager:
                manager._log(INFO, f"Signal {signum} received, initiating shutdown...")
                if hasattr(manager, 'terminate'): # xAppBase standard method
                    manager.terminate() 
                else: # Fallback if terminate not found or not using full xAppBase
                    # Set a simple flag if run_power_management_xapp checks it
                    global_shutdown_flag = True # manager loop would need to check this
                    sys.exit(0) # Force exit if no clean mechanism
            else:
                sys.exit(0)

        if hasattr(manager, 'signal_handler') and callable(manager.signal_handler):
            # If xAppBase provides its own signal_handler, it might already call self.terminate()
            signal.signal(signal.SIGINT, manager.signal_handler)
            signal.signal(signal.SIGTERM, manager.signal_handler)
            if manager and hasattr(manager, '_log'): manager._log(INFO, "Registered signal handlers from xAppBase.")
        else:
            signal.signal(signal.SIGINT, graceful_shutdown)
            signal.signal(signal.SIGTERM, graceful_shutdown)
            if manager and hasattr(manager, '_log'): manager._log(INFO, "Registered custom signal handlers.")
            elif manager: print("INFO: Registered custom signal handlers (logger not available yet).")
