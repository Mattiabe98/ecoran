...
class PowerManager(xAppBase):
    def __init__(self, config_path: str, http_server_port: int, rmr_port: int, kpm_ran_func_id: int = 2):
        ...
        self.adaptive_step_far_thresh_factor = float(self.config.get('adaptive_step_far_threshold_factor', 1.5)) # Name in config
    ...
    def _validate_config(self): # Largely unchanged, uses raise for errors
    ...
    def _get_control_ru_timing_cpu_usage(self) -> float: # Unchanged
        ...
        if not found_valid_core_data and not self.max_ru_timing_usage_history :
            return 0.0 
    ...
    def _run_command(self, cmd_list: List[str]) -> None: # Unchanged (raises RuntimeError on fail)
    ...
    def _setup_intel_sst(self): # Unchanged (raises RuntimeError on fail)
    ...
    def _set_tdp_limit_w(self, tdp_watts: float): # Unchanged (raises RuntimeError on fail)
    ...
    def _adjust_tdp(self, control_ru_cpu_usage: float):
        # Corrected TDP adjustment logic as per original ecoran.py and user's explanation
        ...
        if abs_error_percent > sensitivity_abs_threshold_percent: # Outside deadband
            ...
            # If error_percent > 0: target > actual (actual usage is too LOW). DECREASE TDP to increase stress.
            # If error_percent < 0: target < actual (actual usage is too HIGH). INCREASE TDP to reduce stress.
            if error_percent > 0:
                tdp_change_w = -chosen_step_w  # Decrease TDP
            else: # error_percent <= 0
                tdp_change_w = chosen_step_w   # Increase TDP
            # --- END OF CORRECTION ---
    ...
    def _get_interval_energy_uj(self) -> Optional[float]: # Unchanged
        ...
        if current_energy_uj is None:
            return None
        if self.energy_at_last_log_uj is None:
    ...
    def _kpm_indication_callback(self, e2_agent_id: str, subscription_id: str, indication_hdr_bytes: bytes, indication_msg_bytes: bytes):
        # Added more verbose DEBUG logging for KPM
        print(f"DEBUG KPM CB: Received from {e2_agent_id}, SubID {subscription_id}, HDR len {len(indication_hdr_bytes)}, MSG len {len(indication_msg_bytes)}")
        ...
        try:
            ...
            print(f"DEBUG KPM CB: Extracted meas_report from {e2_agent_id}: {meas_report}") # CRITICAL DEBUG LINE

            if not meas_report:
                print(f"W: Failed to extract KPM measurement data from {e2_agent_id} for SubID {subscription_id}. meas_report is: {meas_report}")
                return
            ...
            with self.kpm_data_lock:
                ...
                print(f"DEBUG KPM CB: Accumulated for {e2_agent_id}: {self.accumulated_kpm_metrics[e2_agent_id]}")
    ...
    def run_power_management_xapp(self): # Main loop largely unchanged, relies on corrected _adjust_tdp
        if os.geteuid() != 0 and not self.dry_run:
            raise SystemExit("Root privileges required for live run.")

        try:
            self.energy_at_last_log_uj = self._read_current_energy_uj() 
            if self.energy_at_last_log_uj is None and not self.dry_run:
            ...
            if self.ru_timing_core_indices:
                ...
                self._update_ru_core_msr_data(); time.sleep(0.2) 
                self._update_ru_core_msr_data(); print("MSR readings primed.")
            else:
                print("INFO: No RU timing cores defined, MSR priming skipped.")

            print("Attempting to set initial TDP based on current PkgWatt...")
            initial_pkg_power, pkg_power_ok = self._get_pkg_power_w(); time.sleep(0.2)
                ...
                if current_time - last_print_time >= self.print_interval_s:
                    ...
                    if self.clos_to_du_names_map:
                        for clos_id in sorted(self.clos_to_du_names_map.keys()):
                            ...
                            clos_dl_b, clos_ul_b = 0.0, 0.0
                            for du_name in du_names_in_clos:
                                gnb_id = self.gnb_ids_map.get(du_name)
                                if gnb_id and gnb_id in current_accumulated_kpm: # Use current_accumulated_kpm for gnb_id check
                                    clos_dl_b += current_accumulated_kpm[gnb_id].get('dl_bits',0.0)
                                    clos_ul_b += current_accumulated_kpm[gnb_id].get('ul_bits',0.0)
                    ...
                    clos_eff_log_str = " | ".join(clos_eff_strs) if clos_eff_strs else "No CLoS DUs"

                    ru_core_usage_list = [f"C{idx}:{self.ru_core_msr_prev_data.get(idx).busy_percent:>6.2f}%" 
                                          if self.ru_core_msr_prev_data.get(idx) else f"C{idx}: N/A  " 
                    ...
                    log_msg_parts = [
                        ...
                        f"IntEgy:{energy_interval_j_str:>5}J", # Shorter name
                        f"TotBits:{total_bits_interval/1e6:.2f}Mb",
        ...
        except RuntimeError as e: print(f"E: Critical runtime error in monitoring loop: {e}")
        except Exception as e: 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--http_server_port", type=int, default=8090, help="HTTP server listen port for xApp.")
    parser.add_argument("--rmr_port", type=int, default=4560, help="RMR port for xApp.")
    ...
    try:
        ...
        if hasattr(manager_instance, 'signal_handler') and callable(manager_instance.signal_handler):
            ...
            signal.signal(signal.SIGQUIT, manager_instance.signal_handler)
            print("INFO: Registered signal handlers from xAppBase.")
        else:
            print("W: No 'signal_handler' method found. Default Python signal handling will apply.")
